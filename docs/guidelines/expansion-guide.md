...
3. **Deploy Command**: Run `mvn deploy` to upload artifacts. Deploy `mvp-core` JAR first, then `server` POM. Ensure the repository is public or the PAT has access. For snapshots, append `-SNAPSHOT` to version in pom.xml.
